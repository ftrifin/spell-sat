...
class ProcedureManagerClass(object):
    ...
    def __processFiles(self, path, spacecraft, procpath, collection, checkLoadable = True ):
        ...
        # Ensure that if there is a problem (e.g. permissions) when reading the dir,
        # file processing will continue
        try:
            filelist = os.listdir(path)
        except:
            LOG("Unable to read directory " + path, LOG_ERROR)
            filelist = []
    ...
    def __isDomainApplicable(self, procedureObject, spacecraft ):
        ...
        # Holds a list of all the procedure families and individual spacecrafts
        procedureSpacecraftFamilies = scList.split(",")
        
        # Trim spaces
        trimmedProcedureSpacecraftFamilies = []
        for sc in procedureSpacecraftFamilies:
            trimmedProcedureSpacecraftFamilies += [ sc.strip() ]
        procedureSpacecraftFamilies = trimmedProcedureSpacecraftFamilies
        
        # If the current spacecraft is in this list, return true
        if (spacecraft in procedureSpacecraftFamilies): return True
        
        # Check now families. Each S/C in the procedure may correspond to a family.
        for sc in procedureSpacecraftFamilies:
            sc = sc.strip()
            family = Config.instance().getSpacecraftFamily(sc)
            if family and spacecraft in family: return True
