...
class ListenerClass(object):
    ...
    def openContext(self, contextName, clientKey = None, warm = False ):
        ...
        if CPP_CONTEXT == True:
            ctxScript = Config.instance().getHome() + os.sep + "bin" + os.sep + "linux" + os.sep + "SPELL-Context "
            ctxScript += " -n " + contextName + " -s " + str(self.__ctxIFC.port)
            ctxScript += " -c " + Config.instance().filename
        else:
            ctxScript = Config.instance().getProperty(LISTENER, "ContextScript")
            ctxScript = Config.instance().getHome() + os.sep + ctxScript
            if not os.path.exists(ctxScript):
                ctxScript += "c" 
