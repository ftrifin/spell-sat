												   ...
												   std::set<std::string> locals)
{
	...
	for( unsigned int index = 0; index<numItems; index++)
	{
		...
		// Do the following check just when the considered variables are not SCDB or GDB or PROC
		if ((varName != "SCDB")&&(varName != "GDB")&&(varName != "PROC")&&(varName != "ARGS"))
		{
