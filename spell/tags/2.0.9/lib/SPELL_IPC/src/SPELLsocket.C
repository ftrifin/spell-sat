...
int SPELLsocket::shutdown()
{
    ...
    return ::shutdown(m_socketFd, SHUT_RDWR);
}
...
SPELLsocket* SPELLsocket::connect( const std::string& server, const int& port )
{
    ...
    if (socketFd< 0)
    {
        throw SPELLipcError("Error on socket creation", errno);
    }
    ...
    if (serverHost == NULL)
    {
        throw SPELLipcError("Error when getting server host name", errno);
    }
    ...
    {
        throw SPELLipcError("Error on socket connect", errno);
    }
...
SPELLsocket* SPELLsocket::listen( int* port )
{
    ...
    {
        throw SPELLipcError("Error on socket configuration (reuse addr)", errno);
    }
    ...
    {
        throw SPELLipcError("Error on socket configuration (linger)", errno);
    }
    ...
    {
        throw SPELLipcError("Error on socket bind", errno);
    }
    ...
    if (getsockname( socketFd, (struct sockaddr*) &serv_addr, &address_len )<0)
    {
        ...
        throw SPELLipcError("Error getting socket address", errno);
    }
    *port = ntohs(serv_addr.sin_port);
    DEBUG("[IPC-SRV-SKT] Listening on port " + ISTR(*port));

    if (::listen(socketFd,5) == -1)
    {
        throw SPELLipcError("Error on listen", errno);
    }
