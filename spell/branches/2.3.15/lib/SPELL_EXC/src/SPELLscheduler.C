        ...
        else
        {
            ...
            else
            {
                ...
                else if (PyDict_Contains( pycond, SSTRPY(Delay) ))
                {
                    ...
                    // Reference counters are incremented inside eval
                    PyObject* theTime = SPELLpythonHelper::instance().eval(delay_time,false);
                    PyObject* theAbsTime = SPELLpythonHelper::instance().eval(abs_time,false);

                    if (theTime != NULL)
                    {
                        SPELLexecutor::instance().getCIF().write("Execution scheduled until time: " + PYREPR(theAbsTime), LanguageConstants::SCOPE_SYS );
