...
void SPELLpythonHelper::updateUserLibrary( PyObject* main_dict, const std::string& libraryFile )
{
    ...
    PyObject* keyList = PyDict_Keys( main_dict );
    int keyCount = PyList_Size( keyList );
    int count = 0;
    for ( count = 0; count < keyCount; count++ )
    {
        PyObject* key = PyList_GetItem( keyList, count );
        if (!PyDict_Contains( moduleDict, key ))
        {
...
PyObject* SPELLpythonHelper::callMethod( PyObject* obj, const std::string& method, ... )
{
    ...
    PyObject* methodC = PyObject_GetAttrString(obj, method.c_str() );
    if (methodC == NULL)
    {
...
SPELLtime SPELLpythonHelper::evalTime( PyObject* expression )
{
    ...
    if (isTime(expression))
    {
        ...
        if (isRel == Py_True)
        {
            PyObject* pySecs = callMethod(expression,"rel",NULL);
            long secs = PyLong_AsLong(pySecs);
        ...
        {
            // New reference
            PyObject* pySecs = callMethod(expression,"abs",NULL);
            int seconds = 0;
            if (PyFloat_Check(pySecs))
            {
                double fsecs = PyFloat_AsDouble(pySecs);
                // New reference
                PyObject* pyDoub = PyLong_FromDouble(fsecs);
                seconds = PyLong_AsLong(pyDoub);
            }
            else
            {
                seconds = PyLong_AsLong(pySecs);
            }
            DEBUG("[PYH] Evaluate time from python object done");
...
PyObject* SPELLpythonHelper::pythonTime( const SPELLtime& time )
{
    ...
    // New reference
    PyObject* instance = PyObject_CallObject( theClass, args );
...
void SPELLpythonHelper::checkError()
{
    ...
    if (err != NULL)
    {
        ...
        else if (excType.find("executor") != std::string::npos )
        {
...
PyObject* SPELLpythonHelper::newInstance( PyObject* classObj, PyObject* argsTuple, PyObject* kwdsDict )
{
    ...
    PyObject* args = argsTuple;
    if (argsTuple == NULL)
    {
        args = PyTuple_New(0);
    }
    // New reference returned
    PyObject* result = PyEval_CallObjectWithKeywords(classObj, args, kwdsDict);
    Py_XINCREF(result);
