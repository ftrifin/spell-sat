...
PyObject* SPELLpyArgs::operator[]( const std::string& modifier ) const
{
    ...
    if (PyDict_Contains( m_config, SSTRPY(modifier) ))
    {
...
bool SPELLpyArgs::hasModifier( const std::string& modifier ) const
{
    ...
    bool keyInDict = (PyDict_Contains( m_config, SSTRPY(modifier))>0);
...
bool SPELLpyArgs::getModifier_PromptUser() const
{
    ...
    if (valueObj != NULL)
    {
        ...
        {
            throw SPELLcoreException("Failed to get boolean value", "Expected a boolean value, obtained: " + PYREPR(valueObj));
        }
    }
    return value;
}
...
std::string SPELLpyArgs::getModifier_ValueFormat() const
{
    ...
    if (formatObj != NULL)
    {
        ...
        {
            std::string format = PYSTR(formatObj);
            if ( format == LanguageConstants::ENG )
            {
                return format;
            }
            else if ( format == LanguageConstants::RAW )
            {
                return format;
            }
            else
            {
                throw SPELLcoreException("Failed to get ValueFormat value", "Expected ENG or RAW constants");
            }
        }
        else
        {
...
std::string SPELLpyArgs::getModifier_Expected() const
{
    ...
    if ( expectedObj != NULL )
    {
        ...
        {
            return PYSTR(expectedObj);
        }
        else
        {
            throw SPELLcoreException("Failed to get Expected value", "Expected a string");
        }
    }
    else
    {
...
std::string SPELLpyArgs::getModifier_Message() const
{
    ...
    if ( valueObj != NULL )
    {
        ...
        {
            return PYSTR(valueObj);
        }
        else
        {
            throw SPELLcoreException("Failed to get Message value", "Expected a string");
        }
    }
    else
    {
...
std::string SPELLpyArgs::getModifier_Procedure() const
{
    ...
    if ( valueObj != NULL )
    {
        ...
        {
            throw SPELLcoreException("Failed to get Procedure value", "Expected a string");
        }
    }
    else
    {
...
int SPELLpyArgs::getModifier_Severity() const
{
    ...
    if ( valueObj != NULL )
    {
        ...
        {
        	int value = PyLong_AsLong(valueObj);
        	switch(value)
        	{
        	...
        	case LanguageConstants::ERROR:
        		return value;
        	default:
        		throw SPELLcoreException("Failed to get Severity value", "Wrong severity code: " + ISTR(value));
        	}
        }
        else
        {
            throw SPELLcoreException("Failed to get Severity value", "Expected a severity code");
        }
    }
    else
    {
        return LanguageConstants::INFORMATION;
    }
}
    ...
    if ( valueObj != NULL )
    {
        if (PyInt_Check( valueObj ) || PyLong_Check( valueObj ))
        {
        	...
        	case LanguageConstants::SCOPE_CFG:
        		return value;
        	default:
        		throw SPELLcoreException("Failed to get Scope value", "Wrong scope code: " + ISTR(value));
        	}
        }
        else
        {
            throw SPELLcoreException("Failed to get Scope value", "Expected a scope code");
        }
    }
    else
    {
        return LanguageConstants::SCOPE_SYS;
    }
}
