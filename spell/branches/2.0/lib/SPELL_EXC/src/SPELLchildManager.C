        ...
        const bool visible )
{
    ...
    for( it = procList.begin(); it != end; it++)
    {
        ...
        std::vector<std::string> tokens = tokenize(id_name, "|");
        std::vector<std::string> path_tokens = tokenize(tokens[0], "/");
    ...
    if (suitableProc == "")
    {
        throw SPELLcoreException("Unable to launch subprocedure " + procId, "No suitable identifier found");
    }
    ...
    {
        throw SPELLcoreException("Unable to start subprocedure " + suitableProc, "Cannot get instance id");
        /** \todo error handling */
    }
    m_childStart.set();
}
