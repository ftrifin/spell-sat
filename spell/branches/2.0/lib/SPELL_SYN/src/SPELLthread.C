// ################################################################################
// FILE       : SPELLthread.C
// DATE       : Mar 17, 2011
// PROJECT    : SPELL
// DESCRIPTION: Implementation of the thread mechanism
// --------------------------------------------------------------------------------
//
//  Copyright (C) 2008, 2011 SES ENGINEERING, Luxembourg S.A.R.L.
//
//  This file is part of SPELL.
//
// SPELL is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// SPELL is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with SPELL. If not, see <http://www.gnu.org/licenses/>.
//
// ################################################################################

// FILES TO INCLUDE ////////////////////////////////////////////////////////
// Local includes ----------------------------------------------------------
#include "SPELL_SYN/SPELLthread.H"
#include "SPELL_SYN/SPELLsyncError.H"
#include "SPELL_SYN/SPELLmonitor.H"
// Project includes --------------------------------------------------------
#include "SPELL_UTIL/SPELLutils.H"
#include "SPELL_UTIL/SPELLlog.H"
// System includes ---------------------------------------------------------


// Bridge for the thread class run() method
void* do_run(void *args)
{
    SPELLthread* thr = reinterpret_cast<SPELLthread*>( args );
    thr->doRun();
    pthread_exit(NULL);
    return 0;
}

//=============================================================================
// CONSTRUCTOR : SPELLthread::SPELLthread
//=============================================================================
SPELLthread::SPELLthread( std::string id )
{
    m_threadId = id;
    m_attr = new pthread_attr_t();
    m_thread = NULL;
    m_running = false;
    pthread_attr_init(m_attr);
    m_detached = false;
    pthread_attr_setdetachstate(m_attr, PTHREAD_CREATE_JOINABLE);
    DEBUG("[THR] Thread " + m_threadId + " created")
}

//=============================================================================
// CONSTRUCTOR : SPELLthread::SPELLthread
//=============================================================================
SPELLthread::SPELLthread( std::string id, bool detached )
{
    m_threadId = id;
    m_attr = new pthread_attr_t();
    m_thread = NULL;
    m_running = false;
    pthread_attr_init(m_attr);
    m_detached = detached;
    if (detached)
    {
        pthread_attr_setdetachstate(m_attr, PTHREAD_CREATE_DETACHED);
    }
    else
    {
        pthread_attr_setdetachstate(m_attr, PTHREAD_CREATE_JOINABLE);
    }
    DEBUG("[THR] Thread " + m_threadId + " created")
}

//=============================================================================
// DESTRUCTOR : SPELLthread::~SPELLthread
//=============================================================================
SPELLthread::~SPELLthread()
{
    DEBUG("[THR] Thread " + m_threadId + " destroyed")
    pthread_attr_destroy(m_attr);
    delete m_attr;
    if (m_thread)
    {
        if (!m_detached) pthread_detach(*m_thread);
        delete m_thread;
        m_thread = NULL;
    }
    m_running = false;
}

//=============================================================================
// METHOD: SPELLthread::start
//=============================================================================
void SPELLthread::start()
{
    m_thread = new pthread_t();
    int result = pthread_create(m_thread, (const pthread_attr_t*) m_attr, do_run, this );
    if( result != 0)
    {
        std::cerr << "THREAD FATAL: CANNOT START " << result << std::endl;
        throw SPELLsyncError("Unable to start thread", "Cannot create thread", result);
    }
    usleep(1000);
}

//=============================================================================
// METHOD: SPELLthread::setRunning
//=============================================================================
void SPELLthread::setRunning( bool running )
{
    if (m_thread)
    {
        SPELLmonitor m(m_lock);
        m_running = running;
    }
}

//=============================================================================
// METHOD: SPELLthread::isRunning
//=============================================================================
bool SPELLthread::isRunning()
{
    SPELLmonitor m(m_lock);
    return (m_running==true);
}

//=============================================================================
// METHOD: SPELLthread::doRun
//=============================================================================
void SPELLthread::doRun()
{
    setRunning(true);
    run();
    setRunning(false);
}

//=============================================================================
// METHOD: SPELLthread::join
//=============================================================================
void SPELLthread::join()
{
    if (!m_detached && (m_thread != NULL) && isRunning())
    {
        int result = pthread_join(*m_thread, NULL);
        if(result != 0)
        {
            throw SPELLsyncError("Unable to join thread", "Pthread join failed", errno);
        }
    }
}
