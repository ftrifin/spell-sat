...
void ClientIF_Install()
{
    ...
    {
        throw SPELLcoreException("Cannot install CIF", "Unable to register object type");
    }
...
static PyObject* ClientIF_Notify( PyObject* self, PyObject* args, PyObject* kwds )
{
    ...
    {
    	LOG_ERROR("Unable to send notification: " + STR(ex.what()));
...
static PyObject* ClientIF_Prompt( PyObject* self, PyObject* args, PyObject* kwds )
{
    ...
    {
    	optlist = PyTuple_GetItem(args,1);
    }
    PyObject* config = NULL;
    if (PyTuple_Size(args)>=3)
    {
    	config = PyTuple_GetItem(args,2);
	}

    // Get the prompt option list, if any
    PromptOptions options;
    if (optlist != NULL)
    {
    	int listSize = PyList_Size(optlist);
    	DEBUG("[CIF PY] Number of prompt options: " + ISTR(listSize))
    	for(int idx=0; idx<listSize; idx++)
    	{
    		PyObject* item = PyList_GetItem(optlist, idx);
    		options.push_back( PYSTR(item) );
    		DEBUG("[CIF PY] Prompt option: " + PYSTR(item))
    	}
    }

    // Get the prompt message
    std::string message = PYSTR(msg);
    // Default configs
	unsigned int scope = LanguageConstants::SCOPE_SYS;
    unsigned int typecode = LanguageConstants::PROMPT_OK_CANCEL;

    // Get the prompt type
    if (config != NULL)
    {
		PyObject* tp = PyDict_GetItemString(config,LanguageModifiers::Type.c_str());
		if ((tp != NULL)&&(Py_None != tp))
		{
			typecode = PyLong_AsLong(tp);
		}

		// Get the prompt scope
		PyObject* sp = PyDict_GetItemString(config,LanguageModifiers::Scope.c_str());
		if ((sp != NULL)&&(Py_None != sp))
		{
			scope = PyLong_AsLong(sp);
		}
    }
    ...
    try
    {
    	result = SPELLexecutor::instance().getCIF().prompt(message, options, typecode, scope);
    }
    catch(SPELLcoreException& ex)
    {
    	LOG_ERROR("Unable to receive prompt answer: " + STR(ex.what()));
    	LOG_ERROR("Will abort execution!");
    ...
    default:
        if (typecode & LanguageConstants::PROMPT_NUM)
        {
        ...
        else if (typecode & LanguageConstants::PROMPT_DATE)
        {
            /** \todo Implement date type in CIF prompt python binding */
        }
        else
        {
