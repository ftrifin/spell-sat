// ################################################################################
// FILE       : SPELLcontextRegistry.C
// DATE       : Mar 17, 2011
// PROJECT    : SPELL
// DESCRIPTION: Context registry implementation
// --------------------------------------------------------------------------------
//
...
std::string SPELLexecutorRegistry::getInstanceId( std::string procId )
{
    ...
    if (m_executorInstances.find(procId) != m_executorInstances.end() )
    {
        ...
        {
            for(InstanceList::iterator it = list.begin(); it != list.end(); it++)
            {
                if ((*it)!=count)
                {
                    instance = count;
...
//=============================================================================
SPELLexecutorManager& SPELLexecutorRegistry::createExecutor( std::string procId )
{
    SPELLmonitor m(m_lock);
    std::string instanceId = getInstanceId( procId );
