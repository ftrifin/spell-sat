...
static PyObject* ClientIF_SetVerbosity( PyObject* self, PyObject* args )
{
    ...
    if (!PyArg_ParseTuple( args, "i", &verbosity))
    {
        ...
        {
            PyErr_Print();
            SPELLpythonHelper::instance().throwSyntaxException("Syntax error on CIF::setVerbosity()", "Malformed argument: " + PYREPR(args));
        }
    }
...
static PyObject* ClientIF_Write( PyObject* self, PyObject* args )
{
    ...
    case ERROR:
        ...
        break;
    default:
        SPELLpythonHelper::instance().throwSyntaxException("Syntax error on CIF::write()", "Unknown severity: " + severity);
    }
...
static PyObject* ClientIF_Prompt( PyObject* self, PyObject* args, PyObject* kwds )
{
    ...
    // Get the prompt type
    int typecode = PROMPT_OK_CANCEL;
    if (config != NULL)
    {
    	PyObject* tp = PyDict_GetItemString(config,Type.c_str());
    ...
    std::string result = cifObject->prompt(message, options, typecode, LanguageConstants::SCOPE_SYS );
    ...
    default:
        ...
        else if (typecode & PROMPT_DATE)
        {
            //TODO (LOW) implement date type in CIF prompt python binding
        }
        else
        {
