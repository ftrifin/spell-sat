...
void SPELLlog::setLogFile( std::string filename, std::string timestamp )
{
    ...
    std::string logfilename = timestamp  + "_Executor_" + filename + ".log";
    // Remove possible ".." and ".py" from file
    replace( logfilename, ".py", "" );
    replace( logfilename, "..", "" );
    replace( logfilename, "//", "/" );
...
void SPELLlog::logInternal( const std::string& location, const std::string& msg, const LogSeverity& sev, const LogLevel& lev )
{
    ...
    if (sev == LOG_WARN || sev == LOG_ERROR || (lev <= m_maxLevel))
    {
        ...
        m_logFile << originStr(location) << "\t" << sevstr << "\t" << timestr << "\t" << levstr
                  << "\t" << msg << std::endl;
        m_logFile.flush();
    }
}
...
void SPELLlog::trace( const std::string& location, const std::string& msg )
{
    ...
    m_logFile << origin << "\t" << sevstr << "\t" << timestr << "\t" << levstr
              << "\t" << msg << std::endl;
    m_logFile.flush();
    std::cout << origin << "  " << sevstr << "  " << timestr << "  " << levstr
              << "  " << msg << std::endl;
}
