...
std::map<std::string, std::vector<BYTE> > SPELLutils::tokenizeData( const std::string& data )
{
    ...
    {
        length = ((unsigned char) data[pos]) * 256 + ((unsigned char) data[pos + 1]);
        pos += 2;
        key.assign(data.begin() + pos, data.begin() + pos + length);
        pos += length;

        DEBUG("Key (size " + ISTR(length) +"): " + key);

        length = ((unsigned char) data[pos]) * 256 + ((unsigned char) data[pos + 1]);
        pos += 2;
        value.assign(data.begin() + pos, data.begin() + pos + length);
        pos += length;

        DEBUGX(tmp.assign(value.begin(), value.end()));
        DEBUG("Value (size " + ISTR(length) +"): " + SPELLutils::dumpString(tmp));
