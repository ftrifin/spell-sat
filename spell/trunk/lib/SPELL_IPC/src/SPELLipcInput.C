...
int SPELLipcInput::readPacketLength( int numRead )
{
    ...
    DEBUGX(tmp.assign(m_buffer, m_buffer + SIZE - 1));
...
{
    DEBUG( NAME + "wait to finish");
    m_finishEvent.wait();
    DEBUG( NAME + "wait to finish done");
}
...
void SPELLipcInput::dispatch()
{
    ...
    case MSG_TYPE_EOC:
        ...
        return;
    default:
    	if ( ((unsigned int) msg.getType()) < MessageType::NumTypes)
    	{
            throw SPELLipcError( NAME + "Unprocessed message type: " + MessageType::TypeStr[msg.getType()], (SPELLerrorCode) IPC_ERROR_UNKNOWN_MSG_TYPE );
    	}
    	else
    	{
    		std::string msgType = ISTR(msg.getType());
            throw SPELLipcError( NAME + "Unknown message type: " + ISTR(msg.getType()), (SPELLerrorCode) IPC_ERROR_UNKNOWN_MSG_TYPE );
    	}
    }
    m_packet = "";
    m_lastByte = NULL;
