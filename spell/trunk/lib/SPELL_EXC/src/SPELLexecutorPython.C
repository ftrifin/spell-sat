...
static PyObject* Executor_GetStatus( PyObject* self, PyObject* args)
{
    ...
    PyObject* statusStr = StatusToPyString(status);
    Py_INCREF(statusStr);
    return statusStr;
}
...
void Executor_Install()
{
    ...
    {
        throw SPELLcoreException("Cannot install executor", "Unable to register object type");
    }
...
static PyObject* Executor_GetChildStatus( PyObject* self, PyObject* args)
{
    ...
    PyObject* status = SSTRPY(StatusToString(st));
    DEBUG("[EXECPY] Get child status: " + StatusToString(st));
    return status;
}
...
static PyObject* Executor_SetUserAction( PyObject* self, PyObject* args )
{
    ...
    if (PyDict_Contains(configObj, SSTRPY(LanguageModifiers::Severity)))
	{
