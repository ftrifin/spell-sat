...
class ExecutorManagerClass(threading.Thread):
    ...
    def processExecutorRequest(self, msg):
        ...
        if msg.getId() == ExcMessages.REQ_NOTIF_EXEC_OPEN:
            ...
            LOG("Executor requested instance id for subprocedure", level = LOG_PROC )
            resp = self.__context.processExecutorRequest(msg)
        else:
    ...
    def processExecutorMessage(self, msg):
        ...
        if msg.getId() == ExcMessages.MSG_NOTIF_EXEC_CLOSE:
            ...
            # If the message is an error, update the executor status
            if msg.getType() == MSG_TYPE_ERROR:
                self.__execStatus = server.executor.status.ERROR
                self.__execError  = msg[FIELD_ERROR]
                self.__execReason = msg[FIELD_REASON]  
            # Forward notification to ALL clients, including parent procedure if any
            if self.getParent() is None:
                result = self.messageToClients(msg)
            else:
                clientList = [self.getParent()] + self.getClients()
                result = self.messageToClients(msg, clientList)
                
            # Process further the message if needed
            if msg.getId() == ExcMessages.MSG_ID_SETUACTION:
                self.__userAction = msg[ExcMessages.FIELD_ACTION_LABEL]
                self.__userActionSeverity = msg[ExcMessages.FIELD_ACTION_SEV]
                self.__userActionEnabled = True
            elif msg.getId() == ExcMessages.MSG_ID_ENABLEUACTION:
                self.__userActionEnabled = True
            elif msg.getId() == ExcMessages.MSG_ID_DISABLEUACTION:
                self.__userActionEnabled = False
            elif msg.getId() == ExcMessages.MSG_ID_DISMISSUACTION:
                self.__userAction = None
                self.__userActionEnabled = False
    ...
    def execConnectionLost(self,key):
        ...
        msg[ExcMessages.FIELD_PROC_ID] = self.__procId
        msg[FIELD_FATAL] = "True"

        # Notify my clients
        clientList = self.getClients()
        
        if self.getParent() is not None:
            clientList += [self.getParent()] 
        self.messageToClients(msg, clientList)
        
        # Notify the rest of clients
        self.__context.notifyExecutorOperation(self.__procId,CtxMessages.DATA_EXOP_CRASH)
    ...
    #===========================================================================
    def getUserActionLabel(self):
        if self.__userAction is None:
            return ""
        return self.__userAction
                    
    #===========================================================================
    def getUserActionEnabled(self):
        return self.__userActionEnabled

    #===========================================================================
    def getUserActionSeverity(self):
        return self.__userActionSeverity
